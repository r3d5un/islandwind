...
func (p *Postgres) SetVersion(version int, dirty bool) error {
	...
	query := `TRUNCATE ` + pq.QuoteIdentifier(
		p.config.migrationsSchemaName,
	) + `.` + pq.QuoteIdentifier(
		p.config.migrationsTableName,
	)
	...
	if version >= 0 || (version == database.NilVersion && dirty) {
		query = `INSERT INTO ` + pq.QuoteIdentifier(
			p.config.migrationsSchemaName,
		) + `.` + pq.QuoteIdentifier(
			p.config.migrationsTableName,
...
func (p *Postgres) Version() (version int, dirty bool, err error) {
	query := `SELECT version, dirty FROM ` + pq.QuoteIdentifier(
		p.config.migrationsSchemaName,
	) + `.` + pq.QuoteIdentifier(
		p.config.migrationsTableName,
	) + ` LIMIT 1`
...
func (p *Postgres) ensureVersionTable() (err error) {
	...
	query = `CREATE TABLE IF NOT EXISTS ` + pq.QuoteIdentifier(
		p.config.migrationsSchemaName,
	) + `.` + pq.QuoteIdentifier(
		p.config.migrationsTableName,
