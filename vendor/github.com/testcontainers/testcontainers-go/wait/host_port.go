...
func (hp *HostPortStrategy) detectInternalPort(
	ctx context.Context,
...
func (hp *HostPortStrategy) WaitUntilReady(ctx context.Context, target StrategyTarget) error {
	...
	if internalPort == "" {
		...
		for internalPort == "" {
			...
			case <-ctx.Done():
				return fmt.Errorf(
					"detect internal port: retries: %d, last err: %w, ctx err: %w",
					i,
					err,
					ctx.Err(),
				)
			case <-time.After(waitInterval):
				if err := checkTarget(ctx, target); err != nil {
					return fmt.Errorf(
						"detect internal port: check target: retries: %d, last err: %w",
						i,
						err,
					)
				}
	...
	for port == "" {
		...
		case <-ctx.Done():
			return fmt.Errorf(
				"mapped port: retries: %d, port: %q, last err: %w, ctx err: %w",
				i,
				port,
				err,
				ctx.Err(),
		...
		case <-time.After(waitInterval):
			if err := checkTarget(ctx, target); err != nil {
				return fmt.Errorf(
					"mapped port: check target: retries: %d, port: %q, last err: %w",
					i,
					port,
					err,
				)
